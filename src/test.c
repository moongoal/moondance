...
static void on_test_complete(
  ...
  for(unsigned i = 0; i < suite->reporter_count; ++i) {
    suite->reporters[i].on_case_complete(&suite->reporters[i], tcase->name, result);
  }
}
...
static void console_result_reporter_on_suite_complete(
  ...
  float const pass_ratio = (test_count > 0 ? (float)data->passed_test_count / test_count : 1.0f);
